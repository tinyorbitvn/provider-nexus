...
import (
	...
	apisCluster "github.com/tinyorbitvn/provider-nexus/apis/cluster"
	apisNamespaced "github.com/tinyorbitvn/provider-nexus/apis/namespaced"
	"github.com/tinyorbitvn/provider-nexus/config"
	"github.com/tinyorbitvn/provider-nexus/internal/clients"
	controllerCluster "github.com/tinyorbitvn/provider-nexus/internal/controller/cluster"
	controllerNamespaced "github.com/tinyorbitvn/provider-nexus/internal/controller/namespaced"
	"github.com/tinyorbitvn/provider-nexus/internal/features"
...
func main() {
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(scheme), "Cannot add cluster-scoped Nexus APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(scheme), "Cannot add namespace-scoped Nexus APIs to scheme")
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Nexus APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Nexus APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-nexus:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Nexus controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Nexus controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Nexus controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Nexus controllers")
	}
